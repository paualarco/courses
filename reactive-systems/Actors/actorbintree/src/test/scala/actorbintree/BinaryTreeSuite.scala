...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  test("GC test") {
    ...
    val ops = List(
      ...
      Contains(requesterRef, id=20, 2),
      Remove(requesterRef, id=30, 1),
      Insert(requesterRef, id=40, 2),
      Insert(requesterRef, id=50, 5),
      Insert(requesterRef, id=60, 4),
      Contains(requesterRef, id=70, 5),
      Remove(requesterRef, id=80, 5),
      Insert(requesterRef, id=90, 6),
      GC,
      Contains(requesterRef, id=100, 5),
      Insert(requesterRef, id=110, 5))


    val expectedReplies = List(
      OperationFinished(id=10),
      ContainsResult(id=20, false),
      OperationFinished(id=30),
      OperationFinished(id=40),
      OperationFinished(id=50),
      OperationFinished(id=60),
      ContainsResult(id=70, true),
      OperationFinished(id=80),
      OperationFinished(id=90),
      CopyFinished,
      ContainsResult(id=100, false),
      OperationFinished(id=110)
    )

    val opsAndReplies = ops zip expectedReplies

    val topNode = requester.system.actorOf(Props[BinaryTreeSet])

    opsAndReplies.foreach { tup =>
      requester.send(topNode, tup._1)
