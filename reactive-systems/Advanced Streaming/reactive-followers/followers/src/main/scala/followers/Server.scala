...
import followers.model.Event.{Follow, StatusUpdate, Unfollow, Broadcast}
...
object Server {
  ...
  /**
    ...
    */
  val reframedFlow: Flow[ByteString, String, NotUsed] = Framing.delimiter(ByteString("\n"), 5000, false).map(_.utf8String)
  ...
  /**
    ...
    */
  val identityParserSink: Sink[ByteString, Future[Identity]] = reframedFlow.take(1).map(Identity.parse).toMat(Sink.head)(Keep.right)
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] = {
    implicit val ordering: Ordering[Event] = Ordering.by(_.sequenceNr)

    def getNextSeqAndEvents(orderedSet: SortedSet[Event]): (Int, SortedSet[Event], SortedSet[Event]) = {
      var nextSeq = orderedSet.head.sequenceNr
      orderedSet.foreach { event =>
        if (event.sequenceNr == nextSeq) nextSeq = nextSeq + 1
      }
      val (outEvents, remainEvents) = orderedSet.span(_.sequenceNr <= nextSeq)
      (nextSeq, outEvents, remainEvents)
    }

    Flow[Event].statefulMapConcat { () =>
      var seqNum = 1
      var sortedEvents: SortedSet[Event] = SortedSet[Event]()
      event =>
        sortedEvents = sortedEvents + event
        if (seqNum == event.sequenceNr) {
          val (nextSeq, outEvents, remainEvents) = getNextSeqAndEvents(sortedEvents)
          seqNum = nextSeq
          sortedEvents = remainEvents
          outEvents.toList
        } else Nil
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] = {
    Flow[Event].statefulMapConcat { () =>
      ...
      event match {
        case f: Follow => {
          println (s"Follow event: $f")
          followers = followers.updated(f.fromUserId, {followers.getOrElse(f.fromUserId, Set[Int]()) + f.toUserId})
        }
        case u: Unfollow => {
