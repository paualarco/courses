...
object SimpleStreaming extends ExtraStreamOps {
   ...
   */
  def filterUsingPreviousFlowAndMapToStringsUsingTwoVias(ints: Source[Int, NotUsed], toString: Flow[Int, String, _]): Source[String, NotUsed] = ints.via(filterEvenValues).via(toString)
   ...
   */
  def recoverSingleElement(ints: Source[Int, NotUsed]): Source[Int, NotUsed] = ints.recover{ case e : IllegalStateException => -1}
  ...
  def recoverToAlternateSource(ints: Source[Int, NotUsed], fallback: Source[Int, NotUsed]): Source[Int, NotUsed] = {
    ints.recoverWith{ case e: IllegalStateException => fallback}
  }
   ...
   */
  def keepRepeatingLastObservedValue: Flow[Int, Int, _] = Flow[Int].extrapolate( Iterator.continually(_), None)
}
